...
class ChessGraph:
    ...
    #def __init__(self, board: chess.Board, graph_type: str = "attack"):
    ...
    def _build_hybrid_graph(self):
        """Constrói grafo híbrido com múltiplas relações."""
    ...
    def to_pytorch_geometric(self) -> Data:
        """Converte o grafo NetworkX para PyTorch Geometric Data."""
        # Extrair nós e arestas
        nodes = list(self.graph.nodes())
        edges = list(self.graph.edges())
        
        # Criar tensores de arestas
        edge_index = torch.tensor(edges, dtype=torch.long).t().contiguous()
        
        # Extrair atributos dos nós
        node_features = []
        for node in nodes:
            piece_info = self.pieces[node]
            features = [
                piece_info.value,
                float(piece_info.color),
                piece_info.position[0] / 7.0,  # Normalizar posição
                piece_info.position[1] / 7.0,
            ]
        ...
        for edge in edges:
            ...
            if edge_data:
                ...
                edge_attr.append([weight, self._relation_to_int(relation)])
            else:
...
class ChessGraphBuilder:
    ...
    def build_from_fen(self, fen: str, graph_type: str = "attack") -> ChessGraph:
        ...
        return ChessGraph(board, graph_type)
    ...
    def build_from_positions(self, positions: List[chess.Board], 
        ...
        for board in positions:
            graph = ChessGraph(board, graph_type)
            graphs.append(graph)
        return graphs
